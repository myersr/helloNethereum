...
namespace etherPure
{
    public class UnitTest1
    {
        ...
        public async Task Test1()
        {
            ...
            //get random number for validity
            Random r = new Random();
            int rInt = r.Next(1, 20); //for ints


            //Numbers will equal 49 if true. 
            var result = await multiplyFunction.CallAsync<int>(rInt);
            Console.WriteLine( rInt + " multiplied by " + 7 + " is: " + result);
            int controlVal = 7*rInt;
            Assert.Equal(controlVal, result);
